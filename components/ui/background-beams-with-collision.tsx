...
export const BackgroundBeamsWithCollision = ({
  ...
  return (
    <div
      ref={parentRef}
      className={cn( "h-96 md:h-[40rem] bg-gradient-to-b from-white to-neutral-100 dark:from-neutral-950 dark:to-neutral-800 relative flex items-center w-full justify-center overflow-hidden", className )}
    >
      {beams.map((beam) => (
        <CollisionMechanism
...
type CollisionMechanismProps = {
  ...
  beamOptions?: {
    initialX?: number; translateX?: number; initialY?: number; translateY?: number; rotate?: number;
    className?: string; duration?: number; delay?: number; repeatDelay?: number;
  };
};

// FIX: Removed the unnecessary React.forwardRef wrapper.
const CollisionMechanism = ({ parentRef, containerRef, beamOptions = {} }: CollisionMechanismProps) => {
...
const Explosion = ({ ...props }: React.HTMLProps<HTMLDivElement>) => {
  ...
  return (
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      ...
      {spans.map((span) => (
        <motion.span key={span.id} initial={{ x: span.initialX, y: span.initialY, opacity: 1 }} animate={{ x: span.directionX, y: span.directionY, opacity: 0 }} transition={{ duration: Math.random() * 1.5 + 0.5, ease: "easeOut" }} className="absolute h-1 w-1 rounded-full bg-gradient-to-b from-indigo-500 to-purple-500" />
      ))}
    </div>
  );
};